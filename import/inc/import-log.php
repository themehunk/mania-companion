...
if ( ! class_exists( 'Themehunk_Importer_Log' ) ) :

	/**
	 * Themehunk Importer
	 */
	class Themehunk_Importer_Log {
		...
		function has_file_read_write() {
			...
			$themehunk_import = wp_nonce_url( admin_url( 'themes.php?page=themehunk-site-library' ), 'themehunk' );
		...
		function start( $data = array(), $demo_api_uri = '' ) {
			...
			Themehunk_Importer_Log::add( 'Started Import Process' );

			Themehunk_Importer_Log::add( '# System Details: ' );
			Themehunk_Importer_Log::add( "Debug Mode \t\t: " . self::get_debug_mode() );
			Themehunk_Importer_Log::add( "Operating System \t: " . self::get_os() );
			Themehunk_Importer_Log::add( "Software \t\t: " . self::get_software() );
			Themehunk_Importer_Log::add( "MySQL version \t\t: " . self::get_mysql_version() );
			Themehunk_Importer_Log::add( "XML Reader \t\t: " . self::get_xmlreader_status() );
			Themehunk_Importer_Log::add( "PHP Version \t\t: " . self::get_php_version() );
			Themehunk_Importer_Log::add( "PHP Max Input Vars \t: " . self::get_php_max_input_vars() );
			Themehunk_Importer_Log::add( "PHP Max Post Size \t: " . self::get_php_max_post_size() );
			Themehunk_Importer_Log::add( "PHP Extension GD \t: " . self::get_php_extension_gd() );
			Themehunk_Importer_Log::add( "PHP Max Execution Time \t: " . self::get_max_execution_time() );
			Themehunk_Importer_Log::add( "Max Upload Size \t: " . size_format( wp_max_upload_size() ) );
			Themehunk_Importer_Log::add( "Memory Limit \t\t: " . self::get_memory_limit() );
			Themehunk_Importer_Log::add( "Timezone \t\t: " . self::get_timezone() );
			Themehunk_Importer_Log::add( PHP_EOL . '-----' . PHP_EOL );
			Themehunk_Importer_Log::add( 'Importing Started! - ' . self::current_time() );

			Themehunk_Importer_Log::add( '---' . PHP_EOL );
			Themehunk_Importer_Log::add( 'WHY IMPORT PROCESS CAN FAIL? READ THIS - ' );
			Themehunk_Importer_Log::add( 'https://themehunk.com/' . PHP_EOL );
			Themehunk_Importer_Log::add( '---' . PHP_EOL );
		 ...
		 */
		public static function log_dir( $dir_name = 'themehunk' ) {
		...
		public static function get_debug_mode() {
			if ( WP_DEBUG ) {
				return __( 'Enabled', 'mania-companion' );
			}

			return __( 'Disabled', 'mania-companion' );
		}
		...
		public static function get_memory_limit() {
			...
			if ( $memory_limit_in_bytes_current < $memory_limit_in_bytes_required ) {
				return sprintf(
					...
					_x( 'Current memory limit %1$s. We recommend setting memory to at least %2$s.', 'Recommended Memory Limit', 'mania-companion' ),
					WP_MEMORY_LIMIT,
					$required_memory
				);
			}
		...
		public static function get_xmlreader_status() {
			...
			if ( class_exists( 'XMLReader' ) ) {
				return __( 'Yes', 'mania-companion' );
			}

			return __( 'No', 'mania-companion' );
		}
		...
		public static function get_php_version() {
			if ( version_compare( PHP_VERSION, '5.4', '<' ) ) {
				return _x( 'We recommend to use php 5.4 or higher', 'PHP Version', 'mania-companion' );
			}
			return PHP_VERSION;
		}
	 ...
	 */
	Themehunk_Importer_Log::get_instance();
